...
| El guardián de anchos solo mira `imagenHero:` | `scripts/verificar-imagenes.mjs` | **4 de los 8 heroes 21/9 a sangre** salen de originales de **898–960 px** y el build pasa limpio, sirviéndolos con `sizes="100vw"` y `priority` → **tarea 1.29** | moderado | bajo |
...
| 1.29 | `scripts/verificar-imagenes.mjs` (`PATRON_HERO`) | Ampliar el guardián de anchos: hoy **solo mira `imagenHero:`**, y por eso **4 de los 8 heroes 21/9 a sangre** salen de originales de **898-960 px** con el build en verde, servidos con `sizes="100vw"` y `priority`. Cubrir también las fotos que `app/proyectos/[slug]/page.tsx:61` sirve a sangre | El script nombra las 4 que hoy pasan limpias. ⚠️ **Fallar el build con ellas dejaría el repo rojo hasta la sesión fotográfica pendiente**: el gate **avisa** mientras esas 4 sigan siendo el material que hay, y pasa a romper el día que lleguen las de 1600 px |
